...
class DecisionsManager extends AbstractManager {
  ...
  readfilter(
    ...
    let queryContent = `SELECT ${this.table}.id, ${this.table}.content, ${this.table}.status, ${this.table}.id_user_creator, ${this.table}.date_created, ${this.table}.date_update, users.firstname, users.lastname FROM ${this.table} INNER JOIN users ON ${this.table}.id_user_creator = users.id AND status ${operator} ? ${durees} ${andUser} ORDER BY ${this.table}.status;`;
    if (userImpacted !== "0") {
    ...
    if (userExpert !== "0") {
      queryContent = `SELECT * FROM ${this.table} WHERE id IN (SELECT decisions_experts.id_decisions FROM decisions_experts WHERE decisions_experts.id_user_expert = ${userExpert} GROUP BY id_decisions) AND status ${operator} ? ${durees};`;
    }
    if (groupImpacted !== "0") {
    ...
    if (groupExpert !== "0") {
      queryContent = `SELECT ${this.table}.id, ${this.table}.content, ${this.table}.status, groups.name FROM ${this.table} INNER JOIN decisions_g_experts ON ${this.table}.id = decisions_g_experts.id_decisions INNER JOIN groups ON groups.id = decisions_g_experts.id_g_expert INNER JOIN group_user ON group_user.id_groups = groups.id INNER JOIN users ON users.id = group_user.id_user WHERE users.id = ${groupExpert} AND status ${operator} ? ${durees};`;
    }
    if (userComment !== "0") {
